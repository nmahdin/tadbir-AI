const fs = require('fs');
let code = fs.readFileSync('src/components/analytics/AnalyticsView.tsx', 'utf8');

// I will rewrite AnalyticsView.tsx completely to give a comprehensive analytics overview covering projects, ideas, content, tasks, etc.

const newAnalytics = `import React, { useState, useMemo } from 'react';
...
export const AnalyticsView: React.FC = () => {
  ...
  const contentTypeData = [
    { name: 'ویدیو/موشن', value: contents.filter(c => c.type === 'video' || c.type === 'motion').length, color: '#f43f5e' },
    { name: 'تصویر/پوستر', value: contents.filter(c => c.type === 'photo' || c.type === 'poster').length, color: '#10b981' },
    { name: 'پادکست', value: contents.filter(c => c.type === 'podcast' || c.type === 'interview').length, color: '#8b5cf6' },
    { name: 'مقاله/خبر', value: contents.filter(c => c.type === 'article' || c.type === 'news' || c.type === 'report').length, color: '#3b82f6' },
  ].filter(d => d.value > 0);
  ...
  const ideasStatusData = [
    ...
    { name: 'تبدیل شده', value: ideas.filter(i => i.status === 'converted').length, color: '#6366f1' },
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-xs">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500">نرخ تکمیل</p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-xs">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500">نرخ انجام</p>
            </div>
          </div>
          <div className="flex items-end justify-between">
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-xs">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500">نرخ تایید</p>
            </div>
          </div>
          <div className="flex items-end justify-between">
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-xs">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500">نرخ انتشار</p>
            </div>
          </div>
          <div className="flex items-end justify-between">
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-xs">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500">مجموع ثبت شده</p>
            </div>
          </div>
          <div className="flex items-end justify-between">
            <span className="text-2xl font-black text-slate-900">{totalLetters}</span>
            <span className="text-sm font-bold text-emerald-600">{respondedLetters} پاسخ‌داده</span>
          </div>
        </div>
      </div>
          ...
          <div className="flex-1 min-h-[250px] relative">
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <div className="text-center">
                <span className="text-2xl font-black text-slate-900">{totalTasks}</span>
                <span className="block text-[10px] text-slate-500 font-bold uppercase mt-1">وظیفه</span>
              </div>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-2 mt-4 pt-4 border-t border-slate-100">
            {statusData.map(item => (
              <div key={item.name} className="flex items-center justify-between text-xs font-bold text-slate-700 bg-slate-50 px-3 py-1.5 rounded-lg">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-xs flex flex-col">
          ...
          <div className="flex-1 min-h-[250px] relative">
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <div className="text-center">
                <span className="text-2xl font-black text-slate-900">{totalContents}</span>
                <span className="block text-[10px] text-slate-500 font-bold uppercase mt-1">محتوا</span>
              </div>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-2 mt-4 pt-4 border-t border-slate-100">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-xs flex flex-col">
          ...
          <div className="flex-1 min-h-[250px] relative">
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <div className="text-center">
                <span className="text-2xl font-black text-slate-900">{totalIdeas}</span>
                <span className="block text-[10px] text-slate-500 font-bold uppercase mt-1">ایده</span>
              </div>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-2 mt-4 pt-4 border-t border-slate-100">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-xs">
          ...
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={teamWorkload} margin={{ top: 20, right: 30, left: 0, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: '#64748b', fontFamily: 'Vazirmatn' }} />
                <YAxis yAxisId="left" orientation="left" axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: '#64748b' }} />
                <YAxis yAxisId="right" orientation="right" axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: '#64748b' }} />
                <Tooltip 
                  cursor={{ fill: '#f8fafc' }}
                  contentStyle={{ borderRadius: '12px', border: '1px solid #e2e8f0', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)', textAlign: 'right', fontFamily: 'Vazirmatn' }}
                />
                <Legend iconType="circle" wrapperStyle={{ fontSize: '12px', paddingTop: '20px', fontFamily: 'Vazirmatn' }} />
                <Bar yAxisId="left" dataKey="avgWorkload" name="میانگین بار کاری (%)" fill="#6366f1" radius={[4, 4, 0, 0]} maxBarSize={40} />
                <Bar yAxisId="right" dataKey="activeTasks" name="وظایف فعال" fill="#cbd5e1" radius={[4, 4, 0, 0]} maxBarSize={40} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
            ...
            {projects.slice(0, 5).map(proj => (
              <div key={proj.id}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-xs font-bold text-slate-600">{proj.progress}%</span>
                </div>
...
fs.writeFileSync('src/components/analytics/AnalyticsView.tsx', newAnalytics);
